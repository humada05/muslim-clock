...
const updateClock = () => {
  ...
  const prayerTrackerCoordinates = (isha < hours || hours < sunrise) ? polarToCartesian(0, 0, biggerRadius, hours) : polarToCartesian(0, 0, smallerRadius, hours);
  ...
  UI.prayer_tracker.setAttribute("transform", `translate(${prayerTrackerCoordinates.x}, ${prayerTrackerCoordinates.y})`)
  
  if (sunrise <= hours && hours < dhuhr) {
    UI.prayer_tracker.setAttribute("fill", 'transparent');
  }
